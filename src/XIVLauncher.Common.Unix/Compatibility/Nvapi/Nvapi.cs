...
{
    public static async Task InstallNvapi(DirectoryInfo prefix, DirectoryInfo installDirectory, string version)
    {
        ...
        CompatToolRelease release = CompatToolbox.GetTool("Nvapi", version);

        var nvapiPath = Path.Combine(installDirectory.FullName, release.Folder, "x64");
        if (!Directory.Exists(nvapiPath))
        {
            ...
            await DownloadNvapi(installPath, release.DownloadUrl).ConfigureAwait(false);
        }
    ...
    private static async Task DownloadNvapi(DirectoryInfo installDirectory, string url)
    {
